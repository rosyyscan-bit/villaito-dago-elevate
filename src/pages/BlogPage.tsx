        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-12 space-y-8">
            {posts.length === 0 ? (
              ...
              posts.map((post) => (
                <Link key={post.id} to={`/blog/${post.slug}`} className="block glass-card rounded-sm overflow-hidden transition-all hover:-translate-y-1">
                  {post.cover_image && (
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="section-padding pt-32">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-6 font-display text-3xl font-bold text-foreground md:text-4xl">{post.title}</h1>
          <p className="mt-2 text-sm text-muted-foreground">{new Date(post.created_at).toLocaleDateString()}</p>
          {post.cover_image && (
            <img src={post.cover_image} alt={post.title} className="mt-6 rounded-sm w-full object-cover" />
          )}
          <div className="mt-8 whitespace-pre-wrap leading-relaxed text-foreground/80">
            {post.content}
          </div>
        </div>
      </div>
