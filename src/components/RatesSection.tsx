  ...
  return (
    <section id="rates" className="section-padding" ref={ref}>
      <div className="mx-auto max-w-4xl text-center">
        ...
        <div className="mt-12 sm:mt-16 grid gap-4 sm:gap-6 md:grid-cols-2">
          {rates.map((rate: any, i: number) => (
            <motion.div
              ...
              className={`border p-6 sm:p-8 text-left transition-colors duration-300 ${
                rate.featured
                  ? "border-primary/30"
                  : "border-border/20 hover:border-primary/20"
              }`}
            >
              <h3 className="font-display text-lg sm:text-xl font-semibold text-foreground">{rate.title}</h3>
              <p className="mt-1 text-xs sm:text-sm text-muted-foreground">{rate.subtitle}</p>
              <div className="mt-4 sm:mt-6">
                <span className="font-display text-2xl sm:text-3xl font-bold text-primary">{rate.price}</span>
                <span className="ml-1 text-xs sm:text-sm text-muted-foreground">{rate.per}</span>
              </div>
              <a
